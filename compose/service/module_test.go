...
func TestModules(t *testing.T) {
	...
	t.Run("labels", func(t *testing.T) {
		...
		t.Run("CRUD", func(t *testing.T) {
			...
			res, err := svc.Create(&types.Module{Name: "unlabeled", NamespaceID: namespaceID})
			req.NoError(unwrapModuleInternal(err))
			req.NotNil(res)
			req.Nil(res.Labels)
			...
			res, err = svc.Create(&types.Module{Name: "labeled", NamespaceID: namespaceID, Labels: map[string]string{"label2": "2nd"}})
			req.NoError(unwrapModuleInternal(err))
			req.NotNil(res)
