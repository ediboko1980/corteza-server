...
func (s Store) LookupLabelByKindResourceIDName(ctx context.Context, kind string, resource_id uint64, name string) (*types.Label, error) {
	return s.execLookupLabel(ctx, squirrel.Eq{
		s.preprocessColumn("lbl.kind", ""):         s.preprocessValue(kind, ""),
		s.preprocessColumn("lbl.rel_resource", ""): s.preprocessValue(resource_id, ""),
		s.preprocessColumn("lbl.name", "lower"):    s.preprocessValue(name, "lower"),
	})
}
...
func (s Store) partialLabelUpdate(ctx context.Context, onlyColumns []string, rr ...*types.Label) (err error) {
	for _, res := range rr {
		...
		err = s.execUpdateLabels(
			ctx,
			squirrel.Eq{
				s.preprocessColumn("lbl.kind", ""): s.preprocessValue(res.Kind, ""), s.preprocessColumn("lbl.rel_resource", ""): s.preprocessValue(res.ResourceID, ""), s.preprocessColumn("lbl.name", "lower"): s.preprocessValue(res.Name, "lower"),
			},
...
func (s Store) DeleteLabelByKindResourceIDName(ctx context.Context, kind string, resourceID uint64, name string) error {
	return s.execDeleteLabels(ctx, squirrel.Eq{
		s.preprocessColumn("lbl.kind", ""):         s.preprocessValue(kind, ""),
		s.preprocessColumn("lbl.rel_resource", ""): s.preprocessValue(resourceID, ""),
		s.preprocessColumn("lbl.name", "lower"):    s.preprocessValue(name, "lower"),
	})
}
